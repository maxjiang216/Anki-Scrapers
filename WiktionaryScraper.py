...
def wiktionary(lang):
    ...
    for word in queue:
        ...
        response = requests.get(
            "https://en.wiktionary.org/wiki/{0}#{1}".format(word, lang)
        )
            ...
            try:
                for ti in soup.find_all(["tr"]):
                    ...
                    if "For pronunciation and definitions of " in temp_trad:
                        ...
                        response = requests.get(
                            "https://en.wiktionary.org/wiki/{0}#{1}".format(
                                trad_word, lang
                            )
                        )
                        soup = bs4.BeautifulSoup(response.content, "html.parser")
        ...
        for i in soup.find_all(["dd", "span", "li", "ol", "p"]):
                ...
                if "mw-headline" in i["class"]:
                    ...
                    if (
                        ...
                        or (
                            temp_active
                            and not active
                            and i.text.lower().rstrip(" 1234567890") in word_classes
                        )
                    ):
                        start = True
                        active = True
                        ...
                        (active or temp_active)
                        and lang not in i["id"]
                        and i.text.lower().rstrip(" 1234567890") not in categories
                    ):
                        if not meanings:
                            print()
                        break
                        ...
                        active
                        and i.text.lower().rstrip(" 1234567890") in categories
                        and active
                        and i.text.lower().rstrip(" 1234567890") not in word_classes
                    ):
                        start = True
                        active = False
                        temp_active = True
            ...
            if active:
                ...
                if (
                    meaning_state
                    and i.text.lower() in categories
                    and i.text.lower() not in word_classes
                ):
                    if "definitions" in i.text.lower():
                        print()
                    meaning_state = False
                ...
                if meaning_state:
                    ...
                    if (
                        pure in meanings
                        or "<br>" + pure + "<br>" in meanings
                        or pure + "<br>" in meanings
                    ):
                        continue
                    if ", trad.]" in pure and ", simp.]" in pure and "[Pinyin]" in pure:
                        ...
                        if (len(exlist) != 4) or "[MSC, " not in pure:
                            continue
                        pure = (
                            exlist[1][: exlist[1].find(" [")]
                            + "<br>"
                            + exlist[2][: exlist[2].find(" [")]
                            + "<br>"
                            + exlist[-1]
                        )
                    ...
                    if (
                        i.name == "dd"
                        and "synonym" not in pure.lower()
                        and "antonym" not in pure.lower()
                    ):
